/* Besoin d'aide ? Envoyez moi un message sur Twitter @phplukaas */

// Et, oui je sais pas besoin de me dire c'est très **très** bordélique comme code mais bon.
	...
	const loadingStages = ["Pré-chargement", "Map", "Addons", "Session"];
	...
	// flemme de tout commenter en detail
	...
	// ouuu les pitis emoji emoji abrico tout ça tu connais surtout la courgette 🍆🍆
	...
	const handlers = {
		startInitFunction(data)
		{
			...
			printLog(1, String.format('Initialisation de {0}', data.type));
			if(data.type) doProgress(data.type);
		},
		startInitFunctionOrder(data)
		{
			count = data.count;
			printLog(1, String.format('[{0}] Initialisation des fonctions ({1} sur {2})', data.type, data.order, data.count));
			if(data.type) doProgress(data.type);
		},
		initFunctionInvoking(data)
		{
			printLog(3, String.format('Appel de {0} sur {1} ({2} sur {3})', data.name, data.type, data.idx, count));
			if(data.type) doProgress(data.type);
		},
		...
		{
			printLog(1, String.format('{0} fonctions on été executées', data.type));
			if(data.type) doProgress(data.type);
		},
		startDataFileEntries(data)
		{
			count = data.count;
			printLog(1, 'Chargement de la carte...');
			if(data.type) doProgress(data.type);
		},
		onDataFileEntry(data)
		{
			printLog(3, String.format('Chargement... {0}', data.name));
			doProgress(data.type);
			if(data.type) doProgress(data.type);
		},
		endDataFileEntries()
		{
			printLog(1, 'Chargement de la map terminée.');
		},
